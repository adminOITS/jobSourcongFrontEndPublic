...
export class HrDashboardComponent {
  ...
  statistics = this.dashboardStatsService.dashboardStatsByCompanyId;
  ...
  constructor() {
    if (this.companyId) {
      this.dashboardStatsService.getDashboardStatsByCompanyId(this.companyId);
      effect(() => {
        if (this.statistics()) {
  ...
  private prepareStatsCards() {
    ...
    this.statsCards = [
      ...
      },
      {
        title: 'TOTAL_MEMBERS',
        value:
          (this.statistics()!.companyStatistics.totalRecruiters ?? 0) +
          (this.statistics()!.companyStatistics.totalValidators ?? 0),
        icon: 'pi pi-building',
